...
// `joy` here is the symlink in `test/node_modules/@symph/joy` which points to the root directory.
// This is done so that requiring from `joy` works.
// The reason we don't import the relative path `../../dist/<etc>` is that it would lead to inconsistent module singletons
import server from '@symph/joy/dist/server/joy'
import _pkg from '@symph/joy/package.json'

export const joyServer = server
export const pkg = _pkg

export function initJoyServerScript (scriptPath, successRegexp, env) {
...
export function renderViaHTTP (url) {
  return fetchViaHTTP(url).then((res) => res.text())
}

export function fetchViaHTTP (url) {
  return fetch(url)
}
...
export function launchApp (dir, port) {
  return runJoyCommandDev([dir, '-p', port])
}

export function joyBuild (dir) {
  return runJoyCommand(['build', dir])
}

export function joyExport (dir, { outdir }) {
  return runJoyCommand(['export', dir, '--outdir', outdir])
}
...
export class Context {
  ...
  getPath (pathname, query) {
    return `${pathname}${query ? `?${qs.stringify(query)}` : ''}`
  }

  getUrl (pathname, query) {
    return `http://localhost:${this.port}${this.getPath(pathname, query)}`
  }
}
