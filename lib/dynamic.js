...
export default function dynamicComponent (p, o) {
  ...
  if (p instanceof SameLoopPromise) {
    ...
    if (!p.modules || !p.render) {
      const errorMessage = '`symphony-joy/dynamic` options should contain `modules` and `render` fields'
      throw new Error(errorMessage)
    }

    if (o) {
      const errorMessage = 'Add additional `symphony-joy/dynamic` options to the first argument containing the `modules` and `render` fields'
      throw new Error(errorMessage)
    }
