...
export default function dynamicComponent (p, o) {
  ...
  if (p instanceof SameLoopPromise) {
    ...
    if (!p.modules || !p.render) {
      const errorMessage = '`@symph/joy/dynamic` options should contain `modules` and `render` fields'
      throw new Error(errorMessage)
    }

    if (o) {
      const errorMessage = 'Add additional `@symph/joy/dynamic` options to the first argument containing the `modules` and `render` fields'
      throw new Error(errorMessage)
    }
    ...
    loadComponent () {
      promise.then((m) => {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(`> symph joy dynamic, can not load component, error: ${e.stack}`)
        }
      })
    }
