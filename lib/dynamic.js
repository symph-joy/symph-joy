...
export default function dynamicComponent (p, o) {
  ...
  if (p instanceof SameLoopPromise) {
    console.log(">>>>>>>>>> p is SameLoopPromise");
    promise = p
    options = o || {}
  } else {
    // Now we are trying to use the modules and render fields in options to load modules.
    if (!p.modules || !p.render) {
      const errorMessage = '`symphony/dynamic` options should contain `modules` and `render` fields'
      throw new Error(errorMessage)
    }

    if (o) {
      const errorMessage = 'Add additional `symphony/dynamic` options to the first argument containing the `modules` and `render` fields'
      throw new Error(errorMessage)
    }
