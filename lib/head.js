...
const JOY_HEAD_IDENTIFIER = 'joy-head'
export function defaultHead (className = JOY_HEAD_IDENTIFIER) {
  return [<meta key={JOY_HEAD_IDENTIFIER} charSet='utf-8' className={className} />]
}
...
function reduceComponents (components) {
  return components
    ...
    .reverse()
    .map((c) => {
      const className = (c.props && c.props.className ? c.props.className + ' ' : '') + JOY_HEAD_IDENTIFIER
    ...
    switch (h.type) {
      ...
      case 'meta':
        for (let i = 0, len = METATYPES.length; i < len; i++) {
          ...
          if (!Object.hasOwnProperty.call(h.props, metatype)) continue
