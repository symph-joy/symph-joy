...
export default class PageLoader {
  ...
  loadPage (route) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (document.getElementById(`__JOY_PAGE__${route}`)) {
        return
      }
  ...
  loadScript (route) {
    ...
    const url = `${this.assetPrefix}/_joy/${encodeURIComponent(this.buildId)}/page${scriptRoute}`
    script.src = url
    script.onerror = () => {
  ...
  clearCache (route) {
    ...
    const script = document.getElementById(`__JOY_PAGE__${route}`)
    if (script) {
