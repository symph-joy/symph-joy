...
export default class PageLoader {
  ...
  loadPage (route) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (document.getElementById(`__SYMPHONY_PAGE__${route}`)) {
        return
      }
  ...
  loadScript (route) {
    ...
    const url = `${this.assetPrefix}/_symphony/${encodeURIComponent(this.buildId)}/page${scriptRoute}`
    script.src = url
    script.onerror = () => {
  ...
  registerPage (route, regFn) {
    ...
    // More info: https://github.com/zeit/symphony.js/pull/1511
  ...
  clearCache (route) {
    ...
    const script = document.getElementById(`__SYMPHONY_PAGE__${route}`)
    if (script) {
