...
export function create(hooksAndOpts = {}, createOpts = {}) {
  ...
  function injectModelClass(createReducer, onError, unlisteners, m) {
    //如果已经存在，就不用重复添加 lane  2017年12月29日
    if(app._models && app._models.length){
      for(let _m of app._models){
        if(_m === m){
    ...
    // set reducers
    store.asyncReducers[m.namespace] = (state = m.state, action) => {
      const { type, nextState } = action;
      if(type === m.namespace+'/setState') {
