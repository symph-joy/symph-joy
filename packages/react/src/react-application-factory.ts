...
import { CoreContainer, CoreFactoryImplement, Type } from "@symph/core";
...
export class ReactApplicationFactoryImplement extends CoreFactoryImplement {
  public async create(entry: Record<string, unknown> | Type = ReactApplicationConfig, options?: JoyApplicationOptions): Promise<ReactApplicationContext> {
    this.applyLogger(options);
    const applicationConfig = new ApplicationConfig();
    const container = new CoreContainer();
    const application = new ReactApplicationContext(entry, applicationConfig, container);
