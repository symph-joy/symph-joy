...
export class ReactRouter<T extends IReactRoute = IReactRoute> {
  ...
  public replaceRouteProvider(nextProvider: ClassProvider, preProviderId: string): { added: T[], removed: T[], modified: T[] } {
    const nextRoutes = (this.scanProvider(nextProvider) || []).map(value => this.extendRoute(value))
    const preProviders = this.filterRoutes(route => route.providerId === preProviderId)
