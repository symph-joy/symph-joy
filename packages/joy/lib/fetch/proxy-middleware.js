...
function createProxyMiddleware(proxyConfig) {
  ...
  if (typeof autoProxy === "boolean") {
    ...
    autoProxyServerConfig = Object.assign(
      {},
      autoProxyDefaultConfig,
      autoProxy
    );
  }
  ...
  for (let i = 0; i < routes.length; i++) {
    ...
    const handler = createProxyServer(route, dev);
    if (handler) {
...
function createProxyServer(route, dev) {
  ...
  const { onProxyReq, onProxyRes, onError } = route;
  ...
  const notifyOnError = (err, req, res) => {
    ...
    if (dev) {
      Log.log(
        `proxy onError, method:${req.method}, url:${req.url}, status:${
          (res && res.statusCode) || "--"
        }}`
      );
    }
  ...
  proxyServer.on("proxyReq", function (proxyReq, req, res, options) {
    ...
    if (dev) {
      Log.log(
        `proxy onReq, method:${req.method}, url:${req.url}, body:${
          req.body || ""
        }`
      );
    }
  ...
  return async function proxy(req, res, next) {
    ...
    const match = pathRegExp.exec(incomingReqUrl.path);
    if (!match) {
    ...
    if (target === "from-header") {
      ...
      if (!target) {
        responseError(
          ...
          res,
          400,
          `${HTTP_HEADER_PROXY_TARGET} header is required`
        );
        return true;
      }
    }
    ...
    const targetUrl = `${target}${outgoingPath}`;
...
// 兼容v1.2.2之前的版本
const createProxyApiMiddleware = createProxyMiddleware;

export { createProxyMiddleware, createProxyApiMiddleware };
