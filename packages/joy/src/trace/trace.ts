...
export class Span {
  ...
  constructor({ name, parentId, attrs, startTime }: { name: string; parentId?: SpanId; startTime?: bigint; attrs?: Object }) {
    this.name = name;
  ...
  stop(stopTime?: bigint) {
    ...
    reporter.report(this.name, Number(duration), Number(timestamp), this.id, this.parentId, this.attrs);
  }
  ...
  manualTraceChild(name: string, startTime: bigint, stopTime: bigint, attrs?: Object) {
