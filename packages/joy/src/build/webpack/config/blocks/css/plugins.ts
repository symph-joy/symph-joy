...
const createLazyPostCssPlugin = (fn: () => import("postcss").AcceptedPlugin): import("postcss").AcceptedPlugin => {
...
async function loadPlugin(dir: string, pluginName: string, options: boolean | object): Promise<import("postcss").AcceptedPlugin | false> {
...
export async function getPostCssPlugins(dir: string, isProduction: boolean, defaults = false): Promise<import("postcss").AcceptedPlugin[]> {
  let config = defaults ? null : await findConfig<{ plugins: CssPluginCollection }>(dir, "postcss");
  ...
  const filtered: import("postcss").AcceptedPlugin[] = resolved.filter(Boolean) as import("postcss").AcceptedPlugin[];
