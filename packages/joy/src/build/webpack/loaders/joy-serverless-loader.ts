...
const joyServerlessLoader: loader.Loader = function () {
  const {
    distDir,
    absolutePagePath,
    page,
  ...
  const normalizeDynamicRouteParams = pageIsDynamicRoute
    ...
    : "";
  const envLoading = `
    const { processEnv } = require('@symph/joy/dist/lib/load-env-config')
    processEnv(${Buffer.from(loadedEnvFiles, "base64").toString()})
  `;

  const runtimeConfigImports = runtimeConfig
    ? `
      const { setConfig } = require('joy/config')
    `
    : "";
  ...
  const dynamicRouteImports = pageIsDynamicRoute
    ? `
    const { getRouteMatcher } = require('joy/dist/joy-server/lib/router/utils/route-matcher');
      const { getRouteRegex } = require('joy/dist/joy-server/lib/router/utils/route-regex');
  `
    : "";
  ...
  const rewriteImports = `
    ...
    const { pathToRegexp, default: pathMatch } = require('joy/dist/joy-server/lib/router/utils/path-match')
  `;

  const handleRewrites = `
    const getCustomRouteMatcher = pathMatch(true)
    const prepareDestination = require('joy/dist/joy-server/lib/router/utils/prepare-destination').default
  ...
  if (page.match(API_ROUTE)) {
    return `
      import initServer from 'joy-plugin-loader?middleware=on-init-server!'
      import onError from 'joy-plugin-loader?middleware=on-error-server!'
      import 'joy/dist/joy-server/server/node-polyfill-fetch'
      ...
      const { apiResolver } = require('joy/dist/joy-server/server/api-utils')
      ${rewriteImports}
    ...
    import onError from 'joy-plugin-loader?middleware=on-error-server!'
    import 'joy/dist/joy-server/server/node-polyfill-fetch'
    const {isResSent} = require('joy/dist/joy-server/lib/utils');
    ...
    const { renderToHTML } = require('joy/dist/joy-server/server/render');
    const { tryGetPreviewData } = require('joy/dist/joy-server/server/api-utils');
    const {sendPayload} = require('joy/dist/joy-server/server/send-payload');
    ...
    export async function renderReqToHTML(req, res, renderMode, _renderOpts, _params) {
      ...
      try {
        ...
        if (parsedUrl.pathname.match(/_joy\\/data/)) {
          const {
            default: getRouteFromAssetPath,
          } = require('@symph/joy/dist/joy-server/lib/router/utils/get-route-from-asset-path');
          _joyData = true;
          parsedUrl.pathname = getRouteFromAssetPath(
            parsedUrl.pathname.replace(
              new RegExp('/_joy/data/${escapedBuildId}/'),
              '/'
            ),
            '.json'
          );
        ...
        const isFallback = parsedUrl.query.__joyFallback

        const previewData = tryGetPreviewData(req, res, options.previewProps)
        const isPreviewMode = previewData !== false

        if (process.env.__JOY_OPTIMIZE_FONTS) {
          renderOpts.optimizeFonts = true
          /**
           * __webpack_require__.__JOY_FONT_MANIFEST__ is added by
           * font-stylesheet-gathering-plugin
           */
          renderOpts.fontManifest = __webpack_require__.__JOY_FONT_MANIFEST__;
          process.env['__JOY_OPTIMIZE_FONT'+'S'] = true
        }
        let result = await renderToHTML(req, res, "${page}", Object.assign({}, getStaticProps ? { ...(parsedUrl.query.amp ? { amp: '1' } : {}) } : parsedUrl.query, nowParams ? nowParams : params, _params, isFallback ? { __joyFallback: 'true' } : {}), renderOpts)

        if (!renderMode) {
          if (_joyData || getStaticProps || getServerSideProps) {
            sendPayload(req, res, _joyData ? JSON.stringify(renderOpts.pageData) : result, _joyData ? 'json' : 'html', ${
...
export default joyServerlessLoader;
