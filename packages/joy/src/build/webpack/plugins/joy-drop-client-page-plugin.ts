...
import { STRING_LITERAL_DROP_BUNDLE } from "../../../joy-server/lib/constants";
...
export class DropClientPage implements WebpackPluginInstance {
  ampPages = new Set();
  ...
  apply(compiler: Compiler) {
    compiler.hooks.compilation.tap(
      ...
      (compilation: any, { normalModuleFactory }: any) => {
        ...
        function findEntryModule(mod: any): Module | null {
        ...
        function handler(parser: any) {
          function markAsAmpFirst() {
            ...
            entryModule.buildInfo.JOY_ampFirst = true;
          }
        ...
        compilation.hooks.seal.tap(PLUGIN_NAME, () => {
          if (isWebpack5) {
            for (const [name, entryData] of compilation.entries) {
              for (const dependency of entryData.dependencies) {
                ...
                if (module?.buildInfo?.JOY_ampFirst) {
          ...
          for (
            ...
            if (entrypoint?.module?.buildInfo?.JOY_ampFirst) {
          ...
          for (let i = compilation.entries.length - 1; i >= 0; i--) {
            ...
            if (entryModule?.buildInfo?.JOY_ampFirst) {
