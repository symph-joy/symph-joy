...
export class JoyBuildService {
  ...
  public async prerenderRoutes(
    ...
    //   const serverBundle = getPagePath(page, outDir);
    //   await promises.unlink(serverBundle);
    // }
    const serverOutputDir = path.join(outDir, SERVER_DIRECTORY);
    ...
    const moveExportedPage = async (
      ...
      // const pagePath = getPagePath(originPage, outDir);
      //
  ...
  public async build(
    ...
    // const telemetry = new Telemetry({ outDir });
    //
    ...
    // );
    //
    // eventNextPlugins(path.resolve(dir)).then((events: any) =>
    ...
    // const prerenderList = await this.prerenderService.getPrerenderList();
    // prerenderList.forEach((info) => {
    //   additionalSsgPaths.set(info.route, info.paths)
    // })
    ...
    //           isLikeServerless ? serverBundle : getPagePath("/_app", outDir),
    //           runtimeEnvConfig,
    //           true
    //         );
    //
    ...
    //     ext: "html" | "json"
    //   ) => {
    //     console.log('>>> moveExportedPage: ', originPage, page, false, isSsg, ext)
    //     // file = `${file}.${ext}`;
    //     // const orig = path.join(exportOptions.outdir, file);
    //     // const pagePath = getPagePath(originPage, outDir);
    //     //
