...
export default function prepareDestination(
  ...
  const destinationCompiler = pathToRegexp.compile(
    destPath,
  ...
  try {
    ...
    if (err.message.match(/Expected .*? to not repeat, but got an array/)) {
      throw new Error(
        `To use a multi-match in the destination you must add \`*\` at the end of the param name to signify it should repeat.`
      );
    }
    throw err;
  }
