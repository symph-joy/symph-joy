...
export function sendPayload(
  ...
  payload: any,
  type: "html" | "json",
  { generateEtags, poweredByHeader }: { generateEtags: boolean; poweredByHeader: boolean },
  options?: { private: true } | { private: boolean; stateful: true } | { private: boolean; stateful: false; revalidate: number | false }
): void {
  if (isResSent(res)) {
    return;
  }
  ...
  if (poweredByHeader && type === "html") {
    res.setHeader("X-Powered-By", "Joy.js");
  }
