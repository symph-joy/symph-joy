...
export class JoyAppConfig implements IJoyConfig, InjectorHookTaps {
  ...
  public mergeCustomConfig(
    customConfig: { [key in keyof Partial<this>]: any }
  ): void {
  ...
  private validateConfig(): boolean {
    if (!isNil(this.experimental)) {
      ...
      if (
        this.experimental?.reactMode &&
        !reactModes.includes(this.experimental.reactMode)
      ) {
        throw new Error(
    ...
    if (!isNil(this.distDir)) {
      if (typeof this.distDir !== "string") {
        throw new Error(
          `Specified distDir is not a string, found type "${typeof this
            .distDir}"`
        );
      }
      ...
      if (userDistDir === "public") {
        throw new Error(
          `The 'public' directory is reserved in Joy.js and can not be set as the 'distDir'. #can-not-output-to-public`
        );
      }
    ...
    if (typeof this.assetPrefix !== "string") {
      throw new Error(
        `Specified assetPrefix is not a string, found type "${typeof this
          .assetPrefix}" #invalid-assetprefix`
      );
    }
    ...
    if (this.basePath !== "") {
      ...
      if (!this.basePath.startsWith("/")) {
        throw new Error(
          `Specified basePath has to start with a /, found "${this.basePath}"`
        );
      }
      ...
      if (this.basePath !== "/") {
        if (this.basePath.endsWith("/")) {
          throw new Error(
            `Specified basePath should not end with /, found "${this.basePath}"`
          );
        }
