...
function assignDefaults(userConfig: { [key: string]: any }) {
  if (typeof userConfig.exportTrailingSlash !== "undefined") {
    console.warn(
      chalk.yellow.bold("Warning: ") +
        'The "exportTrailingSlash" option has been renamed to "trailingSlash". Please update your joy.config.js.'
    );
  ...
  if (typeof result.assetPrefix !== "string") {
    throw new Error(
      `Specified assetPrefix is not a string, found type "${typeof result.assetPrefix}" #invalid-assetprefix`
    );
  }
...
export function normalizeConfig(phase: string, config: any) {
  if (typeof config === "function") {
    ...
    if (typeof config.then === "function") {
      throw new Error(
        "> Promise returned in joy config. #promise-in-joy-config"
      );
    }
  }
  return config;
...
export default function loadConfig(
  ...
  if (path?.length) {
    ...
    if (nonJsPath?.length) {
      throw new Error(
        `Configuring Joy.js via '${basename(
          nonJsPath
