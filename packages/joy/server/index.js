...
export default class Server {
  constructor({
    ...
    if (!dev && !fs.existsSync(resolve(this.distDir, BUILD_ID_FILE))) {
      console.error(
        `> Could not find a valid build in the '${this.distDir}' directory! Try building your app with 'joy build' before starting the server.`
      );
      process.exit(1);
    }
  ...
  async render404(req, res, parsedUrl = parseUrl(req.url, true)) {
    ...
    return this.renderError(
      { statusCode: 404, message: STATUS_CODES[404] },
      req,
      res,
      pathname,
      query
