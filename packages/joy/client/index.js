...
export default async ({ webpackHMR: passedWebpackHMR } = {}) => {
  ...
  try {
    ...
    if (typeof Component !== "function") {
      throw new Error(
        `The default export is not a React Component in Main: "${pathname}"`
      );
    }
  } catch (error) {
  ...
  render({
    App,
    Component,
    appContentProps: props,
    hash,
    err: initialErr,
    emitter,
    Router,
    tempo,
    isComponentDidPrepare: !!initStoreState,
  });
...
export async function renderError(props) {
  ...
  await doRender({
    ...
    appContentProps: initAppContentProps,
  });
}
