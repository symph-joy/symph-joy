...
const symphonyPagesDir = path.join(__dirname, '..', '..', '..', 'pages')
...
export function getPageEntries (pagePaths, {isServer = false, pageExtensions} = {}) {
  ...
  const errorPagePath = path.join(symphonyPagesDir, '_error.js')
  ...
  if (isServer) {
    const documentPagePath = path.join(symphonyPagesDir, '_document.js')
